...
// staying within a configurable token budget. Not safe for concurrent use.
...
func (cb *ContextBuilder) Build(query ParsedQuery) (string, error) {
	...
	summaryBudget := 1000
	if cb.budget < 3000 {
		summaryBudget = cb.budget / 3
	}
	remaining := cb.budget - 2000 - summaryBudget
	if remaining < 0 {
		remaining = cb.budget / 2
		summaryBudget = cb.budget / 4
	}

	var priorityBudget, relevantBudget, broadBudget int
	if query.WatchedOnly {
		// When --watched-only, give most budget to priority (watched) context
		priorityBudget = remaining * 80 / 100
		relevantBudget = remaining * 10 / 100
	...
	// Fetch watch list once for both workspace summary and priority context.
	watchList, err := cb.db.GetWatchList()
	if err != nil {
		return "", fmt.Errorf("fetching watch list: %w", err)
	}

	// 1. Workspace summary
	summary, err := cb.buildWorkspaceSummary(summaryBudget, watchList)
	if err != nil {
	...
	priority, err := cb.buildPriorityContext(query, priorityBudget, seen, watchList)
	if err != nil {
	...
	// 3. Relevant context — query-specific messages or all-channel catchup
	if len(query.Channels) == 0 && len(query.Users) == 0 && len(query.Topics) == 0 && query.TimeRange != nil {
		// Catchup mode: no specific filters, just a time range — load messages from all active channels
		catchup, err := cb.buildCatchupContext(query, relevantBudget, seen)
		if err != nil {
			return "", fmt.Errorf("building catchup context: %w", err)
		}
		if catchup != "" {
			sections = append(sections, catchup)
		}
	} else {
...
// BuildWorkspaceSummary creates a brief overview of the workspace suitable for
// inclusion in the system prompt. Returns workspace stats, channel/user counts,
// and watched items.
func (cb *ContextBuilder) BuildWorkspaceSummary() (string, error) {
	watchList, err := cb.db.GetWatchList()
	if err != nil {
		return "", fmt.Errorf("fetching watch list: %w", err)
	}
	return cb.buildWorkspaceSummary(cb.budget, watchList)
}
...
func (cb *ContextBuilder) buildPriorityContext(query ParsedQuery, budget int, seen map[string]bool, watchList []db.WatchItem) (string, error) {
	if len(watchList) == 0 {
		return "", nil
	}
	...
	for _, w := range watchList {
		...
		if w.EntityType == "channel" {
			...
			if err != nil {
				log.Printf("warning: failed to format channel %s: %v", w.EntityID, err)
				continue
			}
			if section != "" {
	...
	for _, w := range watchList {
		...
		if w.EntityType == "user" {
			...
			if err != nil {
				log.Printf("warning: failed to format user %s: %v", w.EntityID, err)
				continue
			}
			if section != "" {
				b.WriteString(section)
...
func (cb *ContextBuilder) buildRelevantContext(query ParsedQuery, budget int, seen map[string]bool) (string, error) {
	...
	if len(query.Channels) > 0 {
		...
		for _, chName := range query.Channels {
			if channelTokensUsed >= channelBudget {
				break
			}
			...
			from, to := cb.effectiveTimeRange(query)
			section, msgKeys, err := cb.formatChannelMessagesDedup(ch.ID, ch.Name, from, to, channelBudget-channelTokensUsed, seen)
			if err != nil {
				continue
			}
			if section != "" {
				...
				channelTokensUsed += sectionTokens
...
// buildCatchupContext loads recent messages from all active channels for the catchup flow.
// Used when the query has a time range but no specific channels/users/topics.
func (cb *ContextBuilder) buildCatchupContext(query ParsedQuery, budget int, seen map[string]bool) (string, error) {
	from, to := cb.effectiveTimeRange(query)

	// Get active channels sorted by message count
	channelCounts, err := cb.db.GetChannelActivityCounts(from, to, 50)
	if err != nil {
		return "", err
	}
	...
	for _, ch := range channelCounts {
		...
		section, msgKeys, err := cb.formatChannelMessagesDedup(ch.ChannelID, ch.Name, from, to, budget-tokensUsed, seen)
		if err != nil {
			continue
		}
		if section != "" {
...
// sanitizeForPrompt escapes all occurrences of "===" and "---" to prevent
// delimiter injection in the assembled prompt context.
...
func (cb *ContextBuilder) formatMessage(channelName string, msg db.Message) string {
	...
	text := sanitizeForPrompt(msg.Text)
	runes := []rune(text)
	if len(runes) > 500 {
...
func (cb *ContextBuilder) formatThreadSummary(channelID, threadTS, channelName string, budget int) (string, error) {
	...
	start := len(threadReplies) - limit
	for i := start; i < len(threadReplies); i++ {
		reply := threadReplies[i]
		userName, _ := cb.resolveUser(reply.UserID)
		text := sanitizeForPrompt(reply.Text)
		runes := []rune(text)
		if len(runes) > 200 {
