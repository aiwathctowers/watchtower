...
final class InboxFeedbackQueriesTests: XCTestCase {
    ...
    func testRecordNeverShowCreatesMuteRuleWeight1() throws {
        let (pool, item) = try makePool()
        let q = InboxFeedbackQueries(dbPool: pool)
        ...
        XCTAssertEqual(rules[0]["source"] as String, "explicit_feedback")
    }

    // MARK: - record — rule derivation: source_noise

    func testRecordSourceNoiseCreatesMuteRuleWeight08() throws {
        let (pool, item) = try makePool()
        let q = InboxFeedbackQueries(dbPool: pool)
        ...
        XCTAssertEqual(rules.count, 1)
        XCTAssertEqual(rules[0]["rule_type"] as String, "source_mute")
        XCTAssertEqual(rules[0]["weight"] as Double, -0.8)
    }

    // MARK: - record — rule derivation: wrong_class

    func testRecordWrongClassSetsAmbientAndCreatesDowngradeRule() throws {
        let (pool, item) = try makePool()
        let q = InboxFeedbackQueries(dbPool: pool)
        ...
        // Rule should be trigger_downgrade with composite scope key
        let rules = try pool.read { db in
            try Row.fetchAll(db, sql: "SELECT * FROM inbox_learned_rules")
        }
        XCTAssertEqual(rules.count, 1)
        XCTAssertEqual(rules[0]["rule_type"] as String, "trigger_downgrade")
        XCTAssertEqual(rules[0]["scope_key"] as String, "trigger:mention:sender:U002")
        XCTAssertEqual(rules[0]["weight"] as Double, -0.6)
    }

    // MARK: - record — rule derivation: wrong_priority

    func testRecordWrongPriorityCreatesDowngradeRuleOnSender() throws {
        let (pool, item) = try makePool()
        let q = InboxFeedbackQueries(dbPool: pool)
        ...
        XCTAssertEqual(rules[0]["rule_type"] as String, "trigger_downgrade")
        XCTAssertEqual(rules[0]["scope_key"] as String, "sender:U002")
        XCTAssertEqual(rules[0]["weight"] as Double, -0.5)
    }

    // MARK: - record — rule derivation: positive rating

    func testRecordPositiveRatingCreatesBoostRule() throws {
        let (pool, item) = try makePool()
        let q = InboxFeedbackQueries(dbPool: pool)
        try q.record(item: item, rating: 1, reason: "")
        let rules = try pool.read { db in
        ...
        XCTAssertEqual(rules.count, 1)
        XCTAssertEqual(rules[0]["rule_type"] as String, "source_boost")
        XCTAssertEqual(rules[0]["scope_key"] as String, "sender:U002")
        XCTAssertEqual(rules[0]["weight"] as Double, 0.6)
    }

    // MARK: - record — evidence_count increments on repeat feedback
